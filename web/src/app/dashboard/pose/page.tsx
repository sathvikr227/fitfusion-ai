...
import { Camera, CameraOff, RefreshCw, Activity, Info } from "lucide-react"
...
export default function PosePage() {
  ...
  useEffect(() => {
    ...
    function detect(now: number) {
      ...
      const mx = (lm: { x: number; y: number; z: number }) => ({
        x: 1 - lm.x,
        y: lm.y,
        z: lm.z,
      })
      ...
      for (const [i, j] of SKELETON_CONNECTIONS) {
        const a = mx(landmarks[i])
        const b = mx(landmarks[j])
        ctx.beginPath()
      ...
      // Calculate angle at the three key landmarks
      const [li, mi, ri] = def.landmarks
      const a = mx(landmarks[li])
      const b = mx(landmarks[mi])
      const c = mx(landmarks[ri])
      const angle = calcAngle(a, b, c)
      setCurrentAngle(Math.round(angle))

      // Overlay angle on canvas
      const midX = b.x * W
      const midY = b.y * H
      ctx.fillStyle = "#ffffff"
      ctx.font = "bold 20px sans-serif"
      ctx.fillText(`${Math.round(angle)}°`, midX + 12, midY - 8)

      // Rep counting state machine
      let newStage = stageRef.current
      if (def.downCondition(angle) && stageRef.current !== "down") {
        newStage = "down"
      }
      ...
      if (def.downCondition(angle) && stageRef.current === "up") {
        newStage = "down"
      }
      ...
      if (stageRef.current === "down") {
        setFeedback(def.upCondition(angle - 20) ? "Keep going..." : "Good position!")
      } else if (stageRef.current === "up") {
        setFeedback("Rep complete! Return to start.")
      } else {
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">
          Real-time posture detection and automatic rep counting using your camera
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="relative w-full overflow-hidden rounded-3xl bg-slate-900 aspect-video flex items-center justify-center">
            ...
            {isRunning && feedback && (
              <div className="absolute bottom-4 left-1/2 -translate-x-1/2 rounded-2xl bg-black/60 px-4 py-2 text-sm text-white backdrop-blur-sm">
                {feedback}
              </div>
            )}
          </div>
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5">
            <p className="text-xs font-semibold text-slate-500 mb-3">How reps are counted</p>
